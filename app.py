...
from config import APP_NAME, SECRET_KEY, MAX_CONTENT_LENGTH, HOST, PORT, DEBUG, UPLOAD_FOLDER, PROCESSED_FOLDER, GRADCAM_FOLDER, LIME_FOLDER
from utils import allowed_file, save_upload, load_image, save_processed_image, image_info, clear_old_outputs
from inference import predict_with_outputs, get_model, warmup
...
def predict():
    ...
    try:
        ...
        gradcam_name = f"gradcam_{base}.png"
        gradcam_path = os.path.join(GRADCAM_FOLDER, gradcam_name)

        lime_name = f"lime_{base}.png"
        lime_path = os.path.join(LIME_FOLDER, lime_name)
        ...
        return jsonify({
            ...
            "images": {
                ...
                "lime": static_url("lime", lime_name)
            }
        })
